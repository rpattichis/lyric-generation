...
from . import optim
from .lyric_utils import sample_lyric_minibatch
...
class CaptioningSolver(object):
    ...
    def __init__(self, model, data, **kwargs):
        ...
        self.val_batch_size = kwargs.pop("val_batch_size", 100)
    ...
    def _step(self):
        ...
        # idea: use only 1 example per batch from our dataset
        minibatch = sample_lyric_minibatch(
        ...
        # VALIDATION LOSS
        val_minibatch = sample_lyric_minibatch(
            self.data, batch_size=self.val_batch_size, split="val"
        )
        val_captions, val_features = val_minibatch

        # Compute loss and gradient
        val_loss, _ = self.model.loss(val_features, val_captions)
    ...
    def train(self):
        ...
        """
        num_train = self.data["train_lyric"].shape[0]
        ...
        for t in range(num_iterations):
            ...
            if self.verbose and t % self.print_every == 0:
                print(
                    "(Iteration %d / %d) training loss: %f, validation loss: %f"
                    % (t + 1, num_iterations, self.loss_history[-1], self.val_loss_history[-1])
                )
            ...
            if epoch_end:
                ...
                if self.verbose:
                  train_loss = sum(self.loss_history[len(self.loss_history)-iterations_per_epoch:])/iterations_per_epoch
                  val_loss = sum(self.val_loss_history[len(self.val_loss_history)-iterations_per_epoch:])/iterations_per_epoch
                  print(
                      "(Batch #: %f) training loss: %f, validation loss: %f"
