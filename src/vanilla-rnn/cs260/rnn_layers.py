...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # idea: h_t = f_W(h_{t-1}, x_t)
    # turns into: h_t = tanh(W_{hh}*h_{t-1} + W_{xh}*x_t)
...
def rnn_step_backward(dnext_h, cache):
    ...
    # unpack cache and compute gradient of tanh
    x, prev_h, Wx, Wh, b = cache
    dtan = dnext_h * (1 - np.tanh(np.matmul(prev_h, Wh) + np.matmul(x, Wx) + b)**2)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # loop
    for i in range(T):
        h[i], cache_i = rnn_step_forward(xt[i], prev_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dh_curr = dh0 + dh[i]
        dx[i], dh0, dWxi, dWhi, dbi = rnn_step_backward(dh_curr, cache[i])
        dWx += dWxi
        dWh += dWhi
        db += dbi
