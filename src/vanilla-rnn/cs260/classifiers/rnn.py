...
class CaptioningRNN:
    ...
    def __init__(
        ...
        - cell_type: What type of RNN to use; In this homework, we choose 'rnn'.
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == "rnn":
            ...
            raise ValueError(f"{self.cell_type} Not Implemented")
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(1, max_length):
            ...
            #     get scores for all words in the vocabulary                          #
            h0, h2 = next_h.shape
            scores, _ = temporal_affine_forward(next_h.reshape([h0, 1, h2]), W_vocab, b_vocab)
            
            # (4) Select the word with the highest score as the next word, writing it #
            #     (the word index) to the appropriate slot in the captions variable   #
            word_index = np.squeeze(np.argmax(scores, axis=2))
            captions[:, i] = word_index
    ...
    def save(self, filepath):
        """
        Saves the trained parameters necessry to reload the model.
